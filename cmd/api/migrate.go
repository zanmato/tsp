...
func migrateHandType(ctx context.Context, cmd mgx.Commands) error {
	if _, err := cmd.Exec(
		ctx,
		`ALTER TABLE signs ADD COLUMN hand_type TEXT`,
	); err != nil {
		return fmt.Errorf("unable to add column: %w", err)
	}
