...
func (c *SyncClient) loadFromURL(ctx context.Context, dataURL *url.URL) (int, error) {
	...
	for dec.More() {
		...
		// Determine if the entire transcription is within unicode PUA,
		// if not it's invalid
		if !isPUA(s.Transcription) {
			s.Transcription = ""
		}

		// Insert the sign
		batch.Queue(
			`INSERT INTO signs (id, updated_at, video_url, description, unusual, frequency, transcription, vocable, hidden_words, hand_type)
			VALUES ($1, $2, $3, $4, NULLIF($5, '') IS NOT NULL, NULLIF($5, ''), NULLIF($6, ''), NULLIF($7, ''), NULLIF($8, ARRAY['']), NULLIF($9, ''))`,
			id,
			updatedAt,
			baseURL+s.Movie,
			s.Description,
